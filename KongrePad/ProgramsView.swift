...
struct ProgramsView: View {
                ...
                VStack(alignment: .center, spacing: 0) {
                    ...
                    VStack(alignment: .center) {
                        ...
                        VStack {
                            ...
                            // Türkçeye çevrilen tarih
                            if let dayString = programDay?.day, let turkishDate = getTurkishDate(from: dayString) {
                                Text(turkishDate)
                                    .font(.footnote)
                                    .foregroundColor(.black)
                            }
    ...
    func getTurkishDate(from dateString: String) -> String? {
            // API'den gelen format: "31 October Thursday"
            let dateFormatter = DateFormatter()
            dateFormatter.dateFormat = "dd MMMM EEEE"
            dateFormatter.locale = Locale(identifier: "en_US")

            // Tarihi Date formatına çeviriyoruz
            guard let date = dateFormatter.date(from: dateString) else {
                return nil
            }

            // Türkçe formatta gün ve ay gösterimi, yıl yok
            let turkishDateFormatter = DateFormatter()
            turkishDateFormatter.dateFormat = "d MMMM EEEE" // Örn: 31 Ekim, Perşembe
            turkishDateFormatter.locale = Locale(identifier: "tr_TR")

            return turkishDateFormatter.string(from: date).capitalized // Türkçeye çevrilmiş tarih
        }
